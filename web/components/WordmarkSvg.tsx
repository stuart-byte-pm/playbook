/* Playbook Advisory Group — wordmark SVG
   Used in Nav, Footer, and Splash. The `.wm` class controls fill colour
   and is toggled by parent context (scrolled nav, dark footer, etc.). */

interface WordmarkSvgProps {
  className?: string
  role?: string
  ariaLabel?: string
  height?: number
}

export default function WordmarkSvg({
  className = '',
  role = 'img',
  ariaLabel = 'Playbook Advisory Group',
  height,
}: WordmarkSvgProps) {
  return (
    <svg
      className={className}
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 1800 700"
      role={role}
      aria-label={ariaLabel}
      style={height ? { height: `${height}px`, width: 'auto' } : undefined}
    >
      <path className="wm" d="M496.7,403.6v-197.6h-12.8l-51.8,9.7v6.2l2.8,2.5c6.2,4.4,10.3,12.5,10.3,20v159.2c0,7.5-4.1,15.3-10.3,20l-2.8,2.2v6.6h77.7v-6.6l-3.1-2.2c-5.6-4.7-10-12.5-10-20Z"/>
      <path className="wm" d="M676.9,412.9c-2.5,4.4-7.2,4.4-7.2,4.4-6.2,0-8.7-3.1-8.7-11.2v-78.1c0-48.7-39.3-60.9-74.3-60.9s-62.8,15.3-62.8,36.8,10.9,25.3,24.7,25.3,24.7-10.9,24.7-24-3.7-14.7-9.1-19c1.6-1.2,4.7-2.2,9.1-2.2,10.9,0,27.5,6.9,33.7,27.5-2.2,25.9-93.7,19.7-93.7,77.1s22.2,46.8,50.9,46.8,35.6-8.1,45.9-23.7c3.4,15.9,15.3,23.7,32.2,23.7s34.7-14.7,34.7-22.5ZM609.1,372.7c-1.9,18.4-13.1,30.6-24.4,30.6s-19.7-9.7-19.7-25.3c0-26.9,28.4-32.2,44-43.4v38.1Z"/>
      <path className="wm" d="M1444.2,403.4v-19.2c-2.1-47.6-49.8-48.5-51.6-48.5v67.7c0,7.5-4.1,15.3-10.3,20l-2.8,2.2v6.6h77.8v-6.6l-3.1-2.2c-5.6-4.7-10-12.5-10-20Z"/>
      <path className="wm" d="M1543,421.8c-6.2-3.7-12.8-10.3-15.6-17.5l-11.9-29.1c-6.1-15.1-18.3-28.8-32.7-37.2-13.9,4.1-22.1,13-27.1,22.1l24.4,60.4c3.1,6.9,9.7,11.6,17.2,11.6h53.1v-6.6l-7.5-3.7Z"/>
      <path className="wm" d="M1512.6,294.6c5.9-5,14.7-10.6,21.2-13.7l8.7-4.1v-6.6h-53.1v6.6l.3.3c5.3,5.6,4.7,12.5-1.3,17.5l-26.6,21.6c4,4,8.5,6.9,13,9.1l37.7-30.6Z"/>
      <path className="wm" d="M1392.7,244v87.5c45.2,0,50.8-39.6,51.6-49.5v-76.4h-12.8l-51.9,9.7v6.2l2.8,2.5c6.2,4.4,10.3,12.5,10.3,20Z"/>
      <path className="wm" d="M787,282.5h1.3c4.8,1.1,8.5,2.3,11.1,3.6,2.6,1.3,4.3,3.5,5.1,6.5.8,3,0,7.4-2.4,13.1l-29.6,71-30.1-71c-2.2-5.4-3.2-9.6-2.9-12.6.2-2.9,1.6-5.2,4.2-6.9,2.6-1.6,6.6-2.9,12.1-3.8v-8.8h-97.3v8.5c.9.2,1.6.5,2.3,1,4.4,1.3,8.3,3.6,11.9,7,3.6,3.4,7.3,9.3,11.3,17.8l59.4,139-1.6,3.8c-2.3,5.5-5.7,10.5-10.2,15-4.5,4.5-10.6,6.4-18.5,5.7-2.5-.2-5-1-7.3-2.2-2.3-1.3-4.7-2.7-7-4.1-2.3-1.5-4.9-2.8-7.6-4-2.8-1.2-5.7-1.8-8.9-1.8s-7.3,1-11.8,2.9c-5.9,2.8-9.8,7-11.5,12.6-1.7,5.6-1.2,11.2,1.6,16.7,2.8,5.5,7.7,10.2,14.8,14,7.1,3.8,16.4,5.2,27.9,4.1,8.7-.6,16.1-3,22.1-7,6.1-4,11.4-9,15.9-14.8,4.6-5.8,8.5-12.3,11.8-19.4,3.3-7.1,6.3-14.2,9.1-21.2,1.5-3.4,3.8-9.2,7-17.4,2-5.1,4.2-10.7,6.5-16.6l43.4-105.3c3.5-8.9,7.3-15,11.4-18.1,4.1-3.2,9-5.5,14.7-7,.4,0,.9-.1,1.3-.3v-8.8h-59.4v8.8Z"/>
      <path className="wm" d="M1296,267.1c-50.9,0-85.2,33.7-85.2,84.3s34,84,83.7,84,84.6-35,84.6-85.2-32.8-83.1-83.1-83.1ZM1294.4,416.1c-19,0-31.8-25.3-31.8-64s13.1-65.3,32.2-65.3,32.5,25.3,32.5,64.6-13.1,64.6-32.8,64.6Z"/>
      <path className="wm" d="M1119.3,267.1c-50.9,0-85.2,33.7-85.2,84.3s34,84,83.7,84,84.6-35,84.6-85.2-32.8-83.1-83.1-83.1ZM1117.7,416.1c-19,0-31.8-25.3-31.8-64s13.1-65.3,32.2-65.3,32.5,25.3,32.5,64.6-13.1,64.6-32.8,64.6Z"/>
      <path className="wm" d="M948.5,267.1c-18.1,0-30.9,5.6-40,14.7v-75.9h-12.8l-51.8,9.7v6.2l2.8,2.5c6.2,4.4,10.3,12.5,10.3,20v188h12.8l8.4-3.4c2.8-1.2,6.2-1.9,9.7-1.9s8.7.9,13.1,2.8c11.2,4.4,23.4,5.6,34.7,5.6,59.3,0,89.3-33.4,89.3-83.4s-30.9-84.9-76.5-84.9ZM939.8,416.1c-19.4,0-30.3-22.8-31.2-36.5v-35c1.6-35.3,14.7-57.8,33.4-57.8s31.5,25.9,31.5,63.7-13.1,65.6-33.7,65.6Z"/>
      <path className="wm" d="M353.4,267.5c-18.1,0-30.9,5.6-40,14.7v-11.6h-12.8l-51.8,9.7v6.6l2.8,2.2c6.2,4.4,10.3,12.5,10.3,20v159.2c0,7.5-4.1,15.6-10.3,20l-2.8,2.5v6.2h77.7v-6.2l-3.1-2.5c-5.6-4.4-10-12.5-10-20v-50c9.7,11.2,23.7,17.5,42.8,17.5,43.7,0,73.7-33.4,73.7-83.4s-30.9-84.9-76.5-84.9ZM346.2,416.4c-18.4,0-31.2-22.8-32.8-57.8v-14.1c1.6-35,14.7-57.5,33.4-57.5s31.5,26.2,31.5,63.7-13.1,65.6-32.2,65.6Z"/>
    </svg>
  )
}
